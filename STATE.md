...
- `/Volumes/MUSIC/staging/legacy_discard_restore_20260717`: 639 FLACs. Resume
  ...
  live forced-expiry refresh verification.
- **Staging is not complete:** read-only current-byte audit found v4 quick_check
  ok but only 46/639 current SHA256s registered (42/151 + 4/488). Do not reuse
  pre-enrichment staging receipts or execute the mixed root as one release.
  Release-group re-preview is required; classical same-title release collapse
  remains an observed stage-v4 blocker.

## ⛔ KEYSTONE — v4 is the SOLE canonical store (ratified 2026-07-11, spine #189/#190)
Operator-advisor ruling closed the §15↔collapse contradiction (#113/#146). `music_v4.db` is canonical and load-bearing — **NOT inert**; ledger §15/#96 SUPERSEDED (now a pointer stub). No bridge/sync/parity/dual-write to v3. **v3 (`music_v3.db`) will be killed** once the remaining live intake consumer stops touching it. Remaining, in order:
- **(2) intake off v3** — HARD BLOCKER, still v3-native by default. **Bridge Phase 3 + Phase 4 COMPLETE** (spine #203/#205/#208/#209/#212→#213→#214): `ts-stage --v4-intake` (default OFF) reconciles a staged run into v4 via the append-only bridge, AFTER `art + promote`, scoped by `files.original_path`, joining existing releases — additive, v3 still authoritative. Verified on copies (gu_nubreed8 6 create + join, no Jim-Rivers mislink, zero v3 writes). **Phase 4 (slut `06868eed`):** the create branch also projects the enrichment provider-id columns (`files.{spotify,beatport,tidal,qobuz}_id`) into v4 `track_alias` + `source_provenance` — column→row projection, keyed on track_id never title, idempotent. **Phase 4 is now COMPLETE (aliases only): mp3/derivatives RULED OUT of v4 by operator** ("MP3 files, forget they existed", 2026-07-11 → DECISIONS_LOCKED §20; supersedes spec §4 row 5 for the bridge; NOT deferred to Phase 5). **NOT cleared:** flag opt-in/dry-run-default; steady-state still writes v3; alias projection is CREATE-only (no back-fill). Remaining for blocker (2): Phase 5 (soak → default-on → retire `:4314` redirect + `:3644` dual-write). `ts-get` still fully v3-native. **Phase 5 STAGED 2026-07-11 (spine this session):** 5a reproducibility LANDED on `dev` (`97d248a3`) — `docs/v4/migrations/0001_init_v4_schema.sql` gives v4 schema repo-lineage (was NOT rebuildable: schema_migrations had no v4 row, SCHEMA_v4.sql doc'd 11/70 tables), `verify_v4_reproducibility.py` PASS (schema accepts 100% of live catalog FK-clean; NOT data bit-reproducibility). 5b parity soak LANDED on `dev` — `parity_report_v3_v4.py` + report: whole-library bridge apply on copies = missing 1,243→45 (residual untitled/unparseable), 0 dup releases by UPC, 0 FK, zero bridge-induced regression. **5c: default-on MERGED to dev (slut `27957c1e`, operator GO 2026-07-11)** — `--v4-intake` is now default-ON (additive v4 reconcile, best-effort-isolated; branch `phase5c-v4-intake-default-on-STAGED` merged). **Retiring v3 writes is DESIGNED (spine #219, `slut/docs/v4/2026-07-11-v3-write-retire-intake-port-DESIGN.md`), AWAITING operator ratification of approach B.** The bridge is additive (seeds FROM v3 `files`:176) so it CANNOT retire the v3 write; the v4 writer already exists (resolver+bridge), so the retire = **relocate the bridge's INPUT off the v3 files row onto staged FLAC tags+disk (Approach B)**, register/scan/enrich go vestigial. §6's "port deferred until bridge insufficient" clause is triggered (not a reversal). Proposed DECISIONS_LOCKED §21. Gated on stub-master #191/#194 (64 rows OPEN); scope = ts-stage AND ts-get; brain #3 also needed for v3-dead. Until ratified+built, retiring v3 writes is STILL BLOCKED, not staged as code** — intake is v3-native, removing the redirect (intake.py:2521) reopens the F-024 black hole; real prereq = v4-native intake port (#2) + brain re-key (#3). Dual-write already default-OFF (moot). **Guardrail #189/#190: default-on ≠ v3 retired.** FINDING: v3.sha256 ≠ v4.file_hash_sha256 on ~73% (re-tag byte drift) → path, not sha, is the cross-store key for the existing corpus.
- **🟢 IDENTITY-GAP ENRICHMENT 2026-07-12/13 (hag→slut, spine #260/#263, PENDING slut apply):** origin = the pool's Spotify-presence gate (pool_v2/build_pool/overlap_report gate on `spotify_track_id`, which only lives in the frozen crosswalk). Verified: spotify adds nothing ISRC doesn't (every spotify-tagged track has ISRC; 0 the reverse) and the gate needlessly excludes 1,703 ISRC'd tracks. **PENDING DECISION:** re-key the pool gate from spotify→ISRC (kills crosswalk dep + widens pool) — this reframes #257 half-B; still unbuilt. Separately, operator ran the gap tracks through Lexicon Pro (Spotify+Beatport+Tidal, no Premium) + offline Anna's Archive Spotify metadata (`/Volumes/ATTIC/kaggle-data/spotify-aa/tracks.parquet`, 186M ISRCs). hag produced **read-only** enrichment CSVs (`hag/enrich_ALL_noisrc_for_slut.csv` 3,624 track_ids; `hag/enrich_provider_ids_for_slut.csv` 846) matched by title+artist(+duration), source-tagged. No-ISRC gap 4,979→**1,355** open (rest = VA/karaoke/nature/live-bootleg/genre-less classical+arabic, accepted non-ID-able). **slut TODO: apply idempotently (track.isrc where NULL + track_alias spotify/beatport/tidal), sole writer of music_v4.db — hag never wrote.**
- **CORRECTION 2026-07-16:** the Spotify→ISRC "pool re-key" above was derived from the same untracked, off-spec prototypes; it is not a canonical pool-membership decision and is no longer blocker #3. The enrichment CSV handoff remains separate slut-lane identity work.
- **⚠️ SMOKE-TEST 2026-07-12:** a real `ts-get --execute --v4-native-seed` proved **Steps 3/5 are NOT end-to-end reachable** — `--v4-native-seed` is forwarded by **no wrapper** (`tools/get`, `tools/ts-stage` = 0 refs); #230's bash block is at the wrong layer (bypassed by the Python stage pipeline); intake.py's native-seed mode (`:5143`) is right-layer but has **never executed**. v4 was populated by Phase-5c seed-from-v3 (Step 9/9), which DOES work. Downgrade all "Steps 3/5 LANDED/done" claims → "native mode exists, unreachable, unrun." Also: seed-from-staging **create branch throws 21 FK violations while reporting success** (copy-test) — silent defect. Fix is LOW urgency; details: design-doc "SMOKE-TEST RESULT" block. **Separate op finding:** bare `tiddl` resolves to stale `~/Projects/tagslut/.venv/bin/tiddl` (not repo wrapper) → downloads land on internal disk (`~/Music/tiddl`, ALAC) off staging → `prompts/6-tiddl-staging-path-shadow-FIX.md`.
- **FINGERPRINT LAYER — data merged, resolver still OFF (slut `dev`, 2026-07-21):** `48df55fc` fill-only merged the 2026-07-10 sidecar Chromaprint values into `track_file.acoustic_fingerprint` (merge-time receipt: 29,809 written; 928 pre-existing retained; 30,737/32,126 = 95.7%; FK/integrity clean). **Live re-check:** 30,951/32,477 rows currently carry a fingerprint (95.3%; the denominator has since grown). `FINGERPRINT_ENABLED` remains `False`: `_match_fingerprint()` is deliberately a no-op, and the stored base64 Chromaprint values must first be decoded and compared through a candidate pre-filter, calibrated raw-int agreement, and an unambiguous-winner gate. Do not enable by string equality. ISRC remains the byte-independent rule for ordinary tag edits; fingerprint is for the no-ISRC tail.
- **(3) hag off v3 — ✅ CLEARED AND MERGED TO `main` 2026-07-16 (`hag:5d4c204`, ff from `ee839fa`; operator GO; spine #278, ledger §15).** The v4-native identity feeder is now v4-only by construction: `populate_audio_file` has no `--v3-db`/`--v4-only` fork, and the retired `extract_dj_slice` no longer has an environment escape hatch. The real dry-run reached only `music_v4.db`; explicit v3 invocations were rejected before opening a DB. `git grep crosswalk_v3v4_identity -- '*.py'` is empty in tracked hag code. The four root files previously treated as the live re-key half (`pool_v2.py`, `build_pool.py`, `overlap_report.py`, `list_columns.py`) are **untracked, off-spec prototypes**; committed audit `hag/docs/audit/2026-07-08-gemini-pool_v2-unreproducible.md` explicitly says not to promote them. They are not a versioned runtime dependency and must not block v3 retirement. Intake-side retire remains blocked until this branch merges and blocker (2) clears.
<!-- SUPERSEDED 2026-07-16: the following 2026-07-12 block mistook untracked prototypes for versioned runtime. Preserved only as historical provenance.
  - ✅ **FEED half — direct `music_v3.db` reads eliminated.** The v4-native identity feeder ran live (30,473 `audio_file`+`dj_tag`, spine #250, verified); `extract_dj_slice` RETIRED (wrote §12-rejected `musical_key`; superseded by `populate_audio_file --v4-only` + `ingest_mik`, `887808a`/`118c446`); `ingest_automix` crosswalk REPOINTED to v4 `track_alias`+`track_file` (`118c446`); `offline_automix_analyzer` v3 lexicon override RETIRED (`b9a8e4d`). **No brain-feed code opens `music_v3.db`.** Latent-only: `populate_audio_file.py:412` keeps a v3 DEFAULT path (opt-in; `--v4-only` bypasses).
  - ❌ **RE-KEY half — NOT DONE (this is what STATE names #3 around).** `pool_v2.py`, `overlap_report.py`, `build_pool.py`, `list_columns.py` (hag root) still read the `crosswalk_v3v4_identity` **table** — a **FROZEN 2026-07-06 CSV snapshot** (30,507 rows; migration `20260706120000...`; verified spine #254, NOT a live v3 read) carrying both v3 and v4 identity columns. **887808a's "pool query path already v3-independent (spine #247)" is WRONG** (spine #253). The 3 data-readers select ONLY v4-native cols (`v4_file_hash_sha256, spotify_track_id, isrc, title`). **⚠️ #254's "re-key is mechanical / read-path only" framing is FALSE — CORRECTED by spine #257 (advisor-reviewed).** The readers are pure hag-Supabase PostgREST (`rnscghanqopewyfxqjhp`); `track/track_file/track_alias` exist ONLY in slut's local `music_v4.db` and are UNREACHABLE from them. In hag Supabase there is **NO v4-native track-level `spotify_track_id`** (`tag_evidence` is EMPTY; `artist.spotify_artist_id` is artist-level) → the pool gate cannot be fed without a **gated Supabase WRITE**. `isrc`+`title` ARE already v4-native (`dj_tag`, #250, 26,874 isrc). Two paths, BOTH write, staged on operator GO (spine #257 open): **B (lean)** feed `spotify_id` (music_v4 `track_alias`, 27,723) → hag Supabase (`tag_evidence`, keyed by `audio_file_id`, mirrors #250) then re-key readers → dj_tag+tag_evidence (clears grep-DoD; sidesteps 94.1% sha-drift); **C** refresh crosswalk table in place from v4, drop v3 cols (zero reader changes, cheapest, but keeps sha-keyed "v3v4" artifact + fails literal grep-DoD). Coverage receipt: baseline 23,312 spotify pairs → feed via sha=24,290 (+978)/isrc=23,335. Also surfaced: framing DRIFTED from #247(3a) "rebuild crosswalk in place" to #253/#254 "re-key readers" with no stated reason. Real defect beyond liveness = **STALENESS** (snapshot frozen 2026-07-06). → prompt `docs/v4/PROMPT_pool_rekey_blocker3b.md` (its "Risk LOW read-path only" is wrong).
  - **#3 is NOT ratifiable until the re-key half lands.** Branch NOT merged to main (v3-kill gated on all blockers). Intake-side retire (design Step 6) still HARD-BLOCKED until #3 fully clears.
-->
...
Last updated: **2026-07-16** by hag/advisor (blocker #3 corrected against versioned code; hag identity feed made v4-only at `ee839fa`, focused tests + clean-room audit + real v4 dry-run executed; branch pushed, merge pending). Earlier historical sections remain evidence, not current status; KEYSTONE governs.
...
- **SUPERSEDED 2026-07-21:** the separate gated merge into `v4.track_file.acoustic_fingerprint` landed as slut `48df55fc`; see the KEYSTONE fingerprint-layer entry above. The resolver rule remains OFF pending its separately gated matcher/calibration work.
...
- *Recently closed:* **seam transport** — hag consumes the
  ...
  slut's, per `hag:slut_hag_split.md:24`; its former 2% coverage note is historical.
  Current coverage and the still-OFF resolver gate are recorded in the KEYSTONE fingerprint-layer entry.
...
but shipped OFF (the 2% coverage figure is historical; current coverage/gate are recorded in KEYSTONE); Q3 id-map = dissolved (sha256 is the key); Q4 duration-Î =
