...
- **(2) intake off v3** — HARD BLOCKER, still v3-native by default. **Bridge Phase 3 LANDED + Phase 4 ALIAS-HALF ONLY** (spine #203/#205/#208/#209/#212→corrected #213): `ts-stage --v4-intake` (default OFF) reconciles a staged run into v4 via the append-only bridge, AFTER `art + promote`, scoped by `files.original_path`, joining existing releases — additive, v3 still authoritative. Verified on copies (gu_nubreed8 6 create + join, no Jim-Rivers mislink, zero v3 writes). **Phase 4 alias half (slut `06868eed`):** the create branch now also projects the enrichment provider-id columns (`files.{spotify,beatport,tidal,qobuz}_id`) into v4 `track_alias` + `source_provenance` — column→row projection, keyed on track_id never title, idempotent. **⚠️ Phase 4 is NOT complete:** the spec defines Phase 4 as enrichment aliases **AND mp3/derivative writeback (§4 row 5)** — the derivative half is UNBUILT (v3 stores derivatives in `derivative_asset`/`mp3_asset`/`preferred_asset` + `files` mp3 rows; mp3→master linkage design open). Whether derivatives stay in Phase 4 or split to a new phase is an OPEN operator decision, NOT yet ratified. **NOT cleared:** flag opt-in/dry-run-default; steady-state still writes v3; alias projection is CREATE-only (no back-fill). Remaining for blocker (2): Phase 4 derivative half + Phase 5 (soak → default-on → retire `:4314` redirect + `:3644` dual-write). `ts-get` still fully v3-native.
