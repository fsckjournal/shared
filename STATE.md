...
- **(2) intake off v3** — HARD BLOCKER, still v3-native by default. **Bridge Phases 0-4 LANDED** (spine #203/#205/#208/#209): `ts-stage --v4-intake` (default OFF) now reconciles a staged run into v4 via the append-only bridge, AFTER `art + promote`, scoped by `files.original_path`, joining existing releases — additive, v3 still authoritative. Verified on copies (gu_nubreed8 6 create + join, no Jim-Rivers mislink, zero v3 writes by the v4 step). **Phase 4 (slut `06868eed`):** the create branch now also projects the enrichment provider-id columns (`files.{spotify,beatport,tidal,qobuz}_id`, which `index enrich` writes) into v4 `track_alias` + `source_provenance` — a column→row projection (the stage never writes v3 alias *rows*; only `tagslut tag` does), keyed on track_id never title, idempotent. **NOT cleared:** flag is opt-in and dry-run-default; steady-state still writes v3; Phase 4 projects on CREATE only (no back-fill of pre-existing v4 tracks). Remaining to take intake OFF v3 = Phase 5 (mp3/derivatives; soak → default-on → retire the `:4314` redirect + `:3644` dual-write). `ts-get` still fully v3-native (bridge covers `ts-stage` only).
