...
- **(2) intake off v3** — HARD BLOCKER, still v3-native by default. **Bridge Phases 0-3 LANDED** (spine #203/#205/#208/#209): `ts-stage --v4-intake` (default OFF) now reconciles a staged run into v4 via the append-only bridge, AFTER `art + promote`, scoped by `files.original_path`, joining existing releases — additive, v3 still authoritative. Verified on copies (gu_nubreed8 6 create + join, no Jim-Rivers mislink, zero v3 writes by the v4 step). **NOT cleared:** flag is opt-in and dry-run-default; steady-state still writes v3. Remaining to actually take intake OFF v3 = Phases 4-5 (enrichment→track_alias; soak → default-on → retire the `:4314` redirect + `:3644` dual-write). `ts-get` still fully v3-native (Phase 3 covered `ts-stage` only).
