...
Last updated: **2026-07-10** by slut (RADICAL COLLAPSE membership APPLIED to real v4 — agreeing buckets; forks staged; rulings pending — see 🟡 block)

---

## 🟡 2026-07-10 — RADICAL COLLAPSE: membership APPLIED to real v4 (agreeing buckets); forks staged; rulings pending (spine #133/#145)
Split re-verified EXACT vs 07-09: MASTER 25649 / ICEBERG 4695 / BOTH 0 / NEITHER 1101 (31445). #116 "5741 missing" = drain+off-disk, NOT corruption.
- **PRIMARY membership — ✅ APPLIED to real `music_v4.db`** (operator GO, spine #145; `apply_membership.py --apply`). Added `membership TEXT CHECK(work/iceberg/unclassified)` + `membership_reason` + `ix_track_membership`. **work 19560 / iceberg 4695 / unclassified 7190** (1101 off-disk `no_signal` + **6089 `DISAGREE_pending_operator`** HELD, not forced). Snapshot `music_v4.db.bak_pre_membership_20260710_093854`. Tools: `slut/tools/v4/build_membership_evidence.py` + `apply_membership.py` (idempotent, guarded). **Purple #107 is a GHOST on disk** (1 Purple / 40 Orange / 0 iceberg) → filed **#133**, NOT used; MASTER-mass split via listening-artists list (= hag `is_listening_artist` #121). Rule-once set → `slut/tools/v4/output/membership_disagreements.csv`. **PENDING:** 6089 ruling (#133) + purple/orange call + downstream view regen + one-way Supabase mirror of work-set (NOT regenerated — gated on ruling).
- Forks below remain STAGED on `music_v4.collapse_wip.db` (real v4 has no `artist_alias`), await their own GO.
