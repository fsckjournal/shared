...
- **(3) brain re-key** off `crosswalk_v3v4_identity` onto v4 keys (fingerprint coverage 99.6% now supports it; naive v3-full-SHA re-key diverges ~91% — verify). **STILL LIVE 2026-07-12: `crosswalk_v3v4_identity` read by 4 hag files (pool_v2/overlap_report/list_columns/build_pool.py) + `music_v3.db` by 7 more — this is what gates v3-death; a HAG-lane call. Intake-side retire (design Step 6) is HARD-BLOCKED on it.** The one remaining slut-lane intake piece — the `get.py` `_run_local_stage_flow` native-seed fold-in (the URL flow already inherits Step 5 via get-intake) — is DEFERRED behind the owed operator smoke-test (#224/#230), not built (would triplicate an un-smoke-tested reconcile). Design-doc Step 6 line carries the receipts.
