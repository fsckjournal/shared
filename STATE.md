...
- **(3) brain re-key** off `crosswalk_v3v4_identity` onto v4 keys (fingerprint coverage 99.6% now supports it; naive v3-full-SHA re-key diverges ~91% — verify). **HALF DONE 2026-07-12 (hag branch `v3kill-3b-v4-native-feeder`, spine #247-#253):**
  ...
  - ❌ **RE-KEY half — NOT DONE (this is what STATE names #3 around).** `pool_v2.py`, `overlap_report.py`, `build_pool.py`, `list_columns.py` (hag root) still read the `crosswalk_v3v4_identity` **table** — a Supabase snapshot materialized FROM v3 (`is_view=0/is_table=1`), not native v4 keys. **887808a's "pool query path already v3-independent (spine #247)" is WRONG** (spine #253 correction): these read the v3-derived crosswalk. Re-key them onto native v4 (`track_alias.spotify_id` + `track_file.file_hash_sha256` + `track.isrc/title`) → prompt `prompts/9-brain-crosswalk-rekey.md`.
