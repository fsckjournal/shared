...
- **AWAITS operator (Fork 2 decisions, teed up on #144):** (1) **retarget** `ingest_mik` `musical_key`→`key_camelot` — but a full run is a blanket overwrite (ZSONG full-scan + merge-upsert), so fork is **A populate-only** (write kc only where NULL: +5,439 join-recovery, leaves 206 alone — RECOMMENDED, needs a guard) vs **B full overwrite** (force-applies MIK on the 206 = #129 trap, only after 206 ruled). (2) `0`→NULL backfill is **MOOT — 0 live sentinels in brain** (guard `_mik_key` never let them land); close it. Note: only 32 rows carry padded notation (e.g. `05B`), a non-MIK provenance smell per §12.
